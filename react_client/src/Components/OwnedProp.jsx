...
const OwnedProp = ({ id }) => {
  ...
  return (
    <>
      {(space.property && !space.station && !space.utility) && <Property callback={handleClose}/>}
      {(space.station) && <Station callback={handleClose}/>}
      {(space.utility) && <Station callback={handleClose}/>}
      {(space.chance) && <Chance />}
      {(space.comm) && <Community />}
      {(space.tax) && <Tax />}
    </>
  );
};
